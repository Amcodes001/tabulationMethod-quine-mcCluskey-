//Tabulation Method.
//Below code just prints 1 possibility ,
//if many combination of answers possible.
...
//var = no of bits in binary representation.
...
void group1()       //where the minterms are arranged according to the number of ones
{
    ...
    //no of possible groups acc. to no of 1s = var(no of bits in binary represntation of each no.)
    for(i=0;i<=var;i++) 
    {
        temp=save;
        while(temp!=NULL) //process temp till last.
        {
            if(temp->noofones==i) //minterms are arranged according to no. of ones , first 0 ones then 1 ones... and so on
            {
                next->data[0]=temp->data[0]; //Copying Data.
                k++;

                for(j=0;j<var;j++)//Copying Binary Representation.
                {
                    next->bin[j]=temp->bin[j];
                }

                next->noofones=temp->noofones; //Copying No. Of Ones.
                //Allocating Memory for next->right.
...
void variables()            //stores variables(alphabets)
{
    ...
    for(i=0;i<26;i++)
    {
        a[i]=65+i;      //variables(CAPITAL LETTERS)
        b[i]=97+i;      //their compliments(small letters)
    }
}
...
void other_implicants()     //after finding the essential prime implicants other terms necessary are marked
{
    ...
    for(i=0;i<counter;i++)
    {
        ...
        {
            essential[t]=i; //essential[t] = ith row
            count2=count1;
        }
    }
